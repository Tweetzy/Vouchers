        ...
        for (CommandSender sender : to) {
            switch (status) {
                ...
                case UPDATE_AVAILABLE:
                    plugin.getLocale().newMessage(TextUtils.formatText("&dA new update is available for Vouchers")).sendPrefixedMessage(sender);
                    break;
                case UNRELEASED_VERSION:
                    plugin.getLocale().newMessage(TextUtils.formatText(String.format("&dYou're running an unreleased version of Vouchers &f(&c%s&f)", plugin.getDescription().getVersion()))).sendPrefixedMessage(sender);
                    break;
                case UNKNOWN:
