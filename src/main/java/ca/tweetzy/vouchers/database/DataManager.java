...
public final class DataManager extends DataManagerAbstract {
    ...
    public void createVoucher(@NotNull final Voucher voucher, Callback<Voucher> callback) {
        this.runAsync(() -> this.databaseConnector.connect(connection -> {
            final String query = "INSERT INTO " + this.getTablePrefix() + "voucher (id, name, description, item, options, rewards,reward_mode, reward_count) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public void missingColumnsCheck() {
        addColumnToTable("reward_count", "INTEGER", "1");
    }

    public void updateVoucher(@NonNull final Voucher voucher, Callback<Boolean> callback) {
        this.runAsync(() -> this.databaseConnector.connect(connection -> {
            try (PreparedStatement preparedStatement = connection.prepareStatement("UPDATE " + this.getTablePrefix() + "voucher SET name = ?, description = ?, item = ?, options = ?, rewards = ?, reward_mode = ?, reward_count = ? WHERE id = ?")) {
    ...
    public void addColumnToTable(String columName, String dataType, String defaultValue) {
        this.runAsync(() -> this.databaseConnector.connect(connection -> {
            try {
                PreparedStatement ps = connection.prepareStatement("ALTER TABLE " + this.getTablePrefix() + "voucher" + " ADD COLUMN " + columName + " " + dataType + " DEFAULT " + defaultValue);
                ps.execute();
                ps.close();

            } catch (SQLException ignored) {
                //SQLite ignores duplicate column creation by default
            }
        }));
    }
}
