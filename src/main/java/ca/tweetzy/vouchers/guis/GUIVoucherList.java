    ...
    private void draw() {
        ...
        for (Voucher voucher : data) {
            ...
            setAction(slot, ClickType.RIGHT, e -> {
                if (e.player.hasPermission("vouchers.admin")) {
                    e.manager.showGUI(e.player, new GUIVoucherEdit(e.player, voucher.getId(), this));
                }
            });

            setAction(slot, ClickType.LEFT, e -> {
                if (e.player.hasPermission("vouchers.admin")) {
                    PlayerUtils.giveItem(e.player, voucher.getItem(1, true));
                }
            });
